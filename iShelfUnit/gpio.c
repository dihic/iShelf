...
void GPIOInit( void )
{
  ...
  LPC_IOCON->R_PIO1_2  |= 0x01;
	...
	//NodeId_Adress
	//LPC_GPIO[PORT1]->DIR &= ~(0xDC0); //ADDR0-4
	
	GPIOSetDir(PORT1, 0, E_IO_INPUT);	
	GPIOSetDir(PORT1, 1, E_IO_INPUT);	
	GPIOSetDir(PORT1, 2, E_IO_INPUT);
	
	GPIOSetDir(PORT1, 4, E_IO_INPUT);	
	GPIOSetDir(PORT1, 5, E_IO_INPUT);	
	GPIOSetDir(PORT1, 6, E_IO_INPUT);
	GPIOSetDir(PORT1, 7, E_IO_INPUT);
